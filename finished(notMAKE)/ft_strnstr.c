...
/*   Updated: 2023/10/19 20:57:49 by tsimitop         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *ft_strnstr(const char *haystack, const char *needle, size_t len) // FINAL
{
	...
	if (ft_strncmp(needle, "", len_needle) == 0) //if those 2 if's missing it doesn't segfault, without this doesn't work with empty strings
...
// 	i = 0;
// 	len_needle = ft_strlen(needle); // needs to be on top to segfault if it's NULL, pointer doesnt exist
...
// 	needle = "aabc";
// 	len = 5;
// 	printf("%p\t%s\t%s\n", ft_strnstr(haystack, needle, len), haystack, needle);
// 	// printf("%p\t%s\t%s\n", strnstr(haystack, needle, len), haystack, needle);
// 	return (0);
// }